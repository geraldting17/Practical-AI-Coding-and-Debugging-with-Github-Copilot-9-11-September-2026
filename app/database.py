...
_LIST_TICKETS_QUERY = "SELECT * FROM tickets"
_LIST_TICKETS_ORDER = " ORDER BY created_at ASC, id ASC"
_SEED_TICKETS = (
    TicketCreate(
...
class TicketRepository:
    ...
    def list(self, filters: TicketFilters | None = None) -> list[Ticket]:
        ...
        query, parameters = self._build_list_query(filters)
    ...
    @staticmethod
    def _build_list_query(filters: TicketFilters) -> "tuple[str, list[str]]":
        ...
        query = _LIST_TICKETS_QUERY
        if where_parts:
            query += " WHERE " + " AND ".join(where_parts)
        return query + _LIST_TICKETS_ORDER, parameters
    ...
    def _row_to_ticket(row: Iterable[object]) -> Ticket:
        return Ticket.model_validate(dict(zip(_TICKET_COLUMNS, row, strict=True)))
